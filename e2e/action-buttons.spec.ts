...
test('default single mode renders Today and Clear buttons', async ({ page }) => {
    ...
    await expect(p.locator('.drp-date-picker__button--today')).toBeVisible();
    await expect(p.locator('.drp-date-picker__button--clear')).toBeVisible();
});
...
test('clicking Clear empties the input', async ({ page }) => {
    ...
    // Reopen and clear.
    await inputOf(p).click();
    await p.locator('.drp-date-picker__button--clear').click();
...
test('show-today-button="false" omits the Today button', async ({ page }) => {
    ...
    await expect(p.locator('.drp-date-picker__button--today')).toHaveCount(0);
    await expect(p.locator('.drp-date-picker__button--clear')).toBeVisible();
});
...
test('all buttons disabled: the actions container is omitted entirely', async ({ page }) => {
    ...
    await expect(p.locator('.drp-date-picker__actions')).toHaveCount(0);
});
...
test('show-summary="false" omits the summary block in range mode', async ({ page }) => {
    ...
    await expect(p.locator('.drp-date-picker__summary')).toHaveCount(0);
});
...
test('custom formatSummaryCallback overrides the default summary HTML', async ({ page }) => {
    ...
    const summary = p.locator('.drp-date-picker__summary');
    await expect(summary).toHaveClass(/drp-date-picker__summary--visible/);

...
- **Modal positioning mode** — new `positioning-mode="modal"` value alongside `floating` and `inline`. Renders the calendar as a centered overlay with a semi-transparent backdrop scrim instead of anchoring to the input — solves the small-screen multi-month overflow problem where horizontal layouts used to scroll off-screen in floating mode. Closes via backdrop click, Escape, Apply (range mode), or `autoClose='selection'`. Body scroll is locked while open (reference-counted so multiple modal pickers don't fight over `document.body.style.overflow`). Input is `blur()`-ed on open (suppresses mobile soft keyboard) and re-focused on close.
- **Per-tier modal widths + container-query inner layout** — four CSS variables (`--drp-modal-width-xs/sm/md/lg`) align with the `_base.css` 480/768/1200 breakpoints (~95vw on phones, `900px` tablets, `1100px` desktops). Inside the modal, `@container` rules in `_modal.css` collapse to 1/2/3 visible months based on the modal's *actual* width (not viewport width), with hidden columns still updating in lockstep through the collision-resolve logic so range selection across more months still works. Plus shared hooks: `--drp-modal-gap` (16px), `--drp-modal-backdrop-bg`, `--drp-modal-transition`, `--drp-z-index-modal`, `--drp-z-index-modal-backdrop`.
- **Auto-engage modal at small viewports** — two new attributes flip `positioning-mode` from the configured value to `modal` and back as the viewport crosses thresholds: `mobile-modal-breakpoint="640px"` (width) and `mobile-modal-min-height="500px"` (height). Either alone works; both together OR. Implemented with `matchMedia` listeners; only auto-engages when the configured mode is `floating` (pickers explicitly set to `inline` or `modal` are left alone). Uses the reactive `positioning-mode` rebuild path so selection survives the transition (input value persists across rebuild and is re-parsed).
- **Flex-column scroll layout for floating + modal pickers** — calendar is now `display: flex; flex-direction: column; overflow: hidden`; months area is the only scrollable region (`overflow-y: auto; flex: 1; min-height: 0`); header / unified-header / summary / action bar all `flex-shrink: 0`. Side benefit: the action bar (Today / Clear / Apply) is always visible regardless of how tall the multi-month content is. Previously a single `overflow: auto` block let the action bar scroll out of view in floating mode and made days bleed visually behind the action bar in modal mode.
- **Sticky per-month headers + always-render-6-weeks** — per-month header (month name + prev/next, or static label in unified mode) is now `position: sticky; top: 0` within the scrolling months area. Day grid now always renders 42 cells (6×7) regardless of whether the month fits in 5 or 6 weeks — matches the standard convention (Google, Apple, Bootstrap datepicker) and prevents the calendar from jumping in size when navigating between months. Side effect: single-month inline pickers now show one extra row of dimmed `--other-month` days.
- **`show-summary` attribute** — boolean flag to omit the range-mode selection summary block entirely (default `true`). Set to `false` for a clean range picker without the days/nights count line. Previously the only workaround was `formatSummaryCallback = () => ''`, which still rendered an empty `<div>` with margin and border-top causing a small layout jump.
- **Range mode `show()` no longer crashes with partial-range input value when `visible-months-count >= 3`** — `parseAndUpdateSingleDate` built `displayMonths` with at most 2 entries but iterated `visibleMonthsCount` times, so for the 2×3 grid (6 months) or any 3+-month range picker, indices 2+ were `undefined`. **Fix:** build `displayMonths` and `monthDates` with all `visibleMonthsCount` slots upfront, mirroring how single-mode does it.
